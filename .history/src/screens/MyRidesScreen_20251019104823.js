 ...
 * Shows counts consistent with HomeScreen dashboard stats.
 ...
 * - Job counts displayed like HomeScreen dashboard
 ...
 * 
 * Data Sources:
 * - Global app context for jobs array and jobStats from dashboard API
 ...
 * Counts are aligned with HomeScreen dashboard stats.
 * 
...
const MyRidesScreen = ({ navigation, route }) => {
   ...
   * 
   * Returns job count for each tab using jobStats (matches HomeScreen counts).
   * Cancelled jobs are counted directly from jobs array.
   * 
   * @param {string} tabId - Tab ID to get count for
   * @returns {number} Job count for the tab
   */
  const getJobCountByStatus = (tabId) => {
    const statsMap = {
      accepted: jobStats?.accepted || 0,
      pickedup: jobStats?.pickedup || 0,
      delivered: jobStats?.delivered || 0,
      cancelled: jobs.filter(job => mapApiStatusToTabStatus(job.status) === 'cancelled').length,
    };
    return statsMap[tabId] || 0;
  };

  /**
   * Render Tab Button
   * 
   * Renders individual tab buttons with active/inactive styling and job count.
   * 
  ...
  const renderTabButton = (tab) => (
    ...
    >
      <Text style={[
        ...
        {tab.label} ({getJobCountByStatus(tab.id)})  {/* Show count like HomeScreen */}
      </Text>
    </TouchableOpacity>
  );
